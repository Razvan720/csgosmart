...
class UsuariosController {
    ...
    public async updatePass(req: Request, res: Response) {
        ...
        const usuario = await pool.query('SELECT * FROM USUARIOS WHERE usuario=?', [upass.upass_usuario]);
        if (usuario.length == 0){
            res.json({
                'code': '1',
        ...
        if (bcrypt.compareSync(upass.upass_password, usuario[0].password)) {
            ...
            const resultado = await pool.query('UPDATE USUARIOS SET password = ? WHERE id= ? ', [hash,usuario[0].id]);
            res.json({
                'code': '0',
